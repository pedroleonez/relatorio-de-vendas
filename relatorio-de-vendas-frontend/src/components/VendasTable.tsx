...
// Definição da interface baseada nos requisitos do PDF 
interface Venda {
  id: number;
  produto: string;
  categoria: string;
...
const columns = [
  ...
  columnHelper.accessor('categoria', {
    header: 'Categoria',
    cell: info => <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">{info.getValue()}</span>,
  }),
  ...
  columnHelper.accessor('valorTotal', {
    header: 'Total',
    cell: info => new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(info.getValue()),
  }),
          ...
          {table.getRowModel().rows.map(row => (
            <tr key={row.id} className="hover:bg-gray-50 transition-colors">
              {row.getVisibleCells().map(cell => (
                <td key={cell.id} className="whitespace-nowrap px-6 py-4 text-gray-600">
